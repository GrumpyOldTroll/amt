...
#define IP_SFLSIZE(count)                                                  \
    (sizeof(struct ip_sf_socklist) + (count) * sizeof(__u32))
...
#define IGMPV3_MASK(value, nb)                                             \
    ((nb) >= 32 ? (value) : ((1 << (nb)) - 1) & (value))
#define IGMPV3_EXP(thresh, nbmant, nbexp, value)                           \
    ((value) < (thresh)                                                    \
                ? (value)                                                  \
                : ((IGMPV3_MASK(value, nbmant) | (1 << (nbmant + nbexp)))  \
                        << (IGMPV3_MASK((value) >> (nbmant), nbexp) +      \
                                 (nbexp))))

#define IGMPV3_QQIC(value) IGMPV3_EXP(0x80, 4, 3, value)
#define IGMPV3_MRC(value) IGMPV3_EXP(0x80, 4, 3, value)

extern int ip_check_mc(struct in_device* dev,
      u32 mc_addr,
      u32 src_addr,
      u16 proto);
...
extern int ip_mc_source(int add,
      int omode,
      struct sock* sk,
      struct ip_mreq_source* mreqs,
      int ifindex);
extern int ip_mc_msfilter(struct sock* sk,
      struct ip_msfilter* msf,
      int ifindex);
extern int ip_mc_msfget(struct sock* sk,
      struct ip_msfilter* msf,
